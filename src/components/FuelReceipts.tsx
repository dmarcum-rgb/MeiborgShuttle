...
import { supabase, FuelReceipt, Driver } from '../lib/supabase';
import { Plus, Edit, Trash2, X, Fuel, Truck } from 'lucide-react';

type FuelReceiptFormData = {
  driver_id: string;
  date: string;
  amount: string;
  gallons: string;
...
export function FuelReceipts() {
  const [receipts, setReceipts] = useState<(FuelReceipt & { driver: Driver })[]>([]);
  const [drivers, setDrivers] = useState<Driver[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingReceipt, setEditingReceipt] = useState<FuelReceipt | null>(null);
  const [formData, setFormData] = useState<FuelReceiptFormData>({
    driver_id: '',
  ...
  const fetchData = async () => {
    ...
    const [receiptsRes, driversRes] = await Promise.all([
      supabase.from('fuel_receipts').select('*, driver:drivers(*)').order('date', { ascending: false }),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingReceipt) {
        await supabase.from('fuel_receipts').update(data).eq('id', editingReceipt.id);
      } else {
        await supabase.from('fuel_receipts').insert([data]);
      }
      await fetchData();
      closeModal();
    } catch (error) {
      console.error('Error saving fuel receipt:', error);
    } finally {
      setSaving(false);
    }
  };
  ...
  const openModal = (receipt?: FuelReceipt) => {
    if (receipt) {
      ...
      setFormData({
        ...
        gallons: receipt.gallons?.toString() || '',
  ...
  const totalFuel = receipts.reduce((sum, r) => sum + Number(r.amount), 0);
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">Fuel Receipts</h1>
          <p className="text-slate-400 mt-1">Track fuel purchases for all drivers</p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      {drivers.length === 0 && (
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 mb-6">
          <p className="text-amber-400 text-sm">
            No active drivers found. Please add drivers first before logging fuel receipts.
          </p>
        </div>
      )}
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-sm">Total Fuel Expenses</p>
            <p className="text-3xl font-bold text-white">{formatCurrency(totalFuel)}</p>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-900/50">
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Gallons</th>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-700">
              <h2 className="text-xl font-semibold text-white">
                {editingReceipt ? 'Edit Fuel Receipt' : 'Add Fuel Receipt'}
              </h2>
              <button
                onClick={closeModal}
                className="p-2 text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition-all"
